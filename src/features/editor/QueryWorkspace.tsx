...
export const QueryWorkspace = forwardRef<QueryWorkspaceHandle, QueryWorkspaceProps>(function QueryWorkspace(
  ...
  const tables = catalog.objects.map(
    (object): SqlTable => ({
    ...
    <section ref={sectionRef} aria-label="SQL workspace" className="query-workspace">
      ...
      <div className="editor-toolbar">
        <div className="toolbar-group">
          <button className="run-button" type="button">
            <PlayIcon aria-hidden="true" size={14} weight="fill" /> Run query <kbd>Ctrl</kbd><kbd>Enter</kbd>
          </button>
      ...
      <div className={`bottom-panel ${bottomOpen ? "bottom-panel-open" : "bottom-panel-closed"}`}>
        <div className="results-heading">
          <Tabs.Root
            onValueChange={(value) => onUpdatePanel(value as Panel)}
            value={activePanel}
          >
            <Tabs.List aria-label="Query output" className="result-tabs">
              <Tabs.Trigger className="result-tab" onClick={() => onUpdatePanel("results")} value="results">
                Results <span className="tab-count">24,318</span>
              </Tabs.Trigger>
              <Tabs.Trigger className="result-tab" onClick={() => onUpdatePanel("flow")} value="flow">Flow</Tabs.Trigger>
              <Tabs.Trigger className="result-tab" onClick={() => onUpdatePanel("profile")} value="profile">Profile</Tabs.Trigger>
            </Tabs.List>
          </Tabs.Root>
        ...
        {bottomOpen && activePanel === "results" && (
          <div className="result-surface">
            <div className="result-toolbar">
              ...
              <button className="subtle-button" type="button">Copy visible rows</button>
            </div>
            <div className="result-scroll" tabIndex={0}>
              <table>
                <thead>
                  <tr><th>country</th><th>orders</th><th>revenue</th><th>share</th></tr>
                </thead>
                <tbody>
        ...
        {bottomOpen && activePanel !== "results" && (
          <div className="panel-placeholder">
            <strong>{activePanel === "flow" ? "Query flow is ready" : "Profile is ready after execution"}</strong>
            <span>
              {activePanel === "flow"
