...
export function ExportDialog({ projectId, sql, suggestedName }: ExportDialogProps) {
  ...
  async function executeSnapshot(intent: ExportIntent) {
    setSubmitting(true);
    setRequestError(null);
    try {
      if (projectId !== intent.projectId) {
        throw new Error("export.stale: The active project changed before export started.");
      }
      setView(await executeExport(intent.projectId, intent.sql, intent.options));
      setSubmittedSql(intent.sql);
  ...
  async function startExport() {
    ...
    const intent = { projectId, sql, options };
    if (!isClearlyReadOnlySql(intent.sql)) {
      setExportIntent(intent);
      return;
    }
    await executeSnapshot(intent);
  }
    ...
    <>
      ...
      {exportIntent && (
        <ConfirmationDialog
          ...
          description="Export executes this SQL once before writing files and may modify the active project."
          detail={
            exportIntent.sql.length > 600 ? `${exportIntent.sql.slice(0, 600)}…` : exportIntent.sql
          }
          error={requestError}
          onConfirm={() => executeSnapshot(exportIntent)}
          onOpenChange={(nextOpen) => {
            if (!nextOpen) setExportIntent(null);
          }}
          open
          title="Start export?"
          tone="warning"
