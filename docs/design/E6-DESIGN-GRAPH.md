...
At design time the code did not yet match the graph in three important places: engine requests were synchronous, the then-placeholder `arrow-page-format` contained only cursor metadata rather than a concrete page writer/reader, and the result grid was a static fixture without virtualization. E6-T1 through T5 closed those gaps by putting bounded Arrow artifact ownership in the DuckDB adapter and a bounded decoded-page LRU in the desktop. E12-T0 later removed the unused placeholder crate after proving no production consumer remained. The happy path and failure joins are separable because jobs and results are registries with scoped cleanup, and cancellation was implemented before page/grid work was accepted.
