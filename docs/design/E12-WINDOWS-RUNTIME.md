       ...
       └─→ probe_webview2 (1)
           ...
           └─ 🔒 native runtime response → RuntimePrerequisite
              └─→ create_isolated_profile (1) @runtime-smoke
                  R: runner temporary directory
                  E: directory creation|environment isolation failure ↯escape(non-zero; preserve diagnostics)
                  ├─→ launch_portable (N) @desktop-launch
                  │   R: extracted Tarik.exe, redirected APPDATA/LOCALAPPDATA/TEMP
...
BOUNDARIES: Release ZIP, manifest, and checksum text are untrusted until exact-content, PE, schema, and digest checks pass; WebView2 discovery output is untrusted until it reports a non-empty native version; CIM process records are admitted only when rooted in the launched Tarik PID tree; JSON sidecar frames are parsed and matched to the requested ID; startup logs are evidence only when read from the redirected profile; manual results become trusted only with OS build, DPI/monitor setup, artifact digest, date, and reviewer recorded.
...
TEST LAYERS: Pure Node tests provide temporary fake PE files, checksums, manifests, process snapshots, and workload reports; native `windows-latest` provides the extracted release, WebView2, CIM, redirected AppData, and real sidecar; manual review provides physical Windows 10/11 machines and display/input configurations. The graph and report schema remain unchanged across layers.
