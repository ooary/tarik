...
**Status:** `IN PROGRESS / T1 REVIEW / T3-T4 CANDIDATE / T5 REVIEW PACKET` — The user approved the E15.1 task plan and durable T0 design on September 11, 2026. The T1 setup engine, T2 portable guidance, T3 destination grants, and T4 guarded complete-query exports are committed locally; the consolidated T5 review packet is prepared. The combined connection-assistant, optional-guidance, destination-policy, and export-approval UI remains an uncommitted review candidate. T0/T1/T3/T4/T5 completion still requires native Windows evidence and/or explicit real-Tauri/manual sign-off as documented. E15 consolidated review remains independently open.
...
- [ ] **E15.1-T3 Add reusable export destination grants**
  - Status: Linux-developed implementation candidate. Schema/repository/path policy, private bridge listing, public listing-only MCP tool, typed desktop commands, native picker flow, and destination policy UI are implemented. Backend/protocol/docs are committed as `d3862f5`; the UI remains uncommitted pending real-Tauri and native Windows review.
  - Depends on: E15.1-T0 approval
...
- [ ] **E15.1-T5 Package and review the assisted connection and export workflow**
  - Status: Consolidated review packet prepared at `docs/review/E15-1-CONNECTION-EXPORT.md`. Automated Linux/package evidence is complete; local review artifacts truthfully record `sourceDirty: true` because postponed UI remains uncommitted. Native Windows and real-Tauri/manual sign-off rows remain open, so T5 is unchecked.
