...
**Status:** `IN PROGRESS / T1 REVIEW / T3 CANDIDATE / T4 CANDIDATE` — The user approved the E15.1 task plan and durable T0 design on September 11, 2026. The T1 setup engine and T2 portable guidance are committed locally; the T3 destination-grant and T4 guarded complete-query export candidates are implemented. The combined connection-assistant, optional-guidance, destination-policy, and export-approval UI remains an uncommitted review candidate. T0/T1 completion still requires native Windows host/config/process evidence and explicit real-Tauri UI sign-off. E15 consolidated review remains independently open.
