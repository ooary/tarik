...
- [ ] **E13-T6 Run desktop UX, Results, branding, and process-lifecycle acceptance**
  ...
  - Notes:
    ...
    - September 7, 2026 packaged editor correction: Tauri's generated CSP nonce was not passed to CodeMirror, so WebView2 rejected the runtime `style-mod` stylesheet and rendered stacked gutters plus browser-default autocomplete. `SqlEditor` now supplies `EditorView.cspNonce`; direct packaged CDP verification confirms the 188-rule CodeMirror stylesheet is active, multiline gutter/code baselines match, editor typing works, and autocomplete has its normal selected-row, border, scrolling, and icon styling. Evidence: `target/verify-packaged-codemirror-after-fix.json` and `target/packaged-code-mirror-after-fix.png`; replacement archive SHA-256 `d2649c3a37833fc60d965e9c41f9f253590e2d055915faac2197bcb988ddc8ca`.
...
- [x] **E16-T1 Add bounded owner-scoped result leases and discovery**
  - Depends on: E16-T0
  ...
  - Notes: Implemented bridge protocol v4 with profile-scoped SafeRead query/result authority, up to eight retained results and four outstanding queries per profile under global count/byte reservations, writer-side 32 MiB result enforcement, exact published cache bytes, truthful browse-cap metadata, reserve/restore/consume snapshot admission, actionable blocking IDs, idempotent release, and bounded `tarik_list_active`. Real-sidecar tests prove same-profile reconnect can discover/page/release retained results while foreign ownership remains denied. Desktop-custom limit persistence is completed with the Activity/settings unit; this task establishes the immutable effective-limit DTO and default admission snapshot.

- [x] **E16-T2 Add lease expiry and race-safe cleanup**
  - Depends on: E16-T1
  ...
  - Notes: Added authenticated 30-second adapter heartbeats, 120-second connection leases, independent five-second desktop maintenance, and a bounded resume-safe grace that never converts a stale heartbeat into a fresh one. Result leases expire after 10 minutes idle or 30 minutes absolute; authority is removed before cleanup, in-flight page reads hold explicit leases, failed/locked deletion remains charged and retries with bounded exponential backoff, and a 16-entry cleanup backlog applies admission backpressure. Disconnect cancels live origin work while same-profile retained results survive reconnect; revoke, Analyze removal, project invalidation, disable, shutdown, explicit release, and scoped desktop Release all invalidate results without touching project data or completed exports. Startup’s existing owned-cache reconciliation removes stale result artifacts without restoring authority or rerunning SQL.

- [x] **E16-T3 Queue queries fairly under shared execution admission**
  ...
  - Notes: SafeRead starts now reserve a bounded immutable admission record before a round-robin paired-profile scheduler chooses the oldest query for the next profile. One MCP heavy query is engine-accepted at a time; grant, active project, registered sources, SQL classification, and catalog revision are revalidated at claim. Snapshots remain reserved while queued, are consumed only after engine acceptance, and are restored after queue deadline or eligible pre-accept failure. A desktop-owned five-second watchdog enforces independent 60-second queue and execution deadlines without MCP polling; queued cancellation is terminal immediately while running cancellation remains requested until engine confirmation. DuckDB query workers now clone the live session connection at worker claim, with a regression proving queued SQL observes preceding catalog changes. Existing engine admission continues to keep imports and profiles exclusive and per-session query execution serial; broader desktop/export/quality priority remains conservatively bounded by the engine’s existing busy/resource gates rather than introducing a second connection or false parallelism.

- [x] **E16-T4 Teach hosts multi-result workflow and recovery**
  ...
  - Notes: Updated initialization instructions, tool descriptions, all seven static prompts’ shared contract, the packaged Agent Skill, MCP setup guide, and user guide. Guidance now treats queue/run/cancel/terminal/result/expiry/cleanup as distinct, uses `tarik_list_active` for same-profile ID recovery, follows actionable blocking IDs, starts raw exploration with selected columns plus `LIMIT 100`, avoids bulk paging, reports row-capped results as incomplete/non-exact, and hands off in order to refine/aggregate, user-controlled non-executing Open in editor, or guarded complete-query export (prefer Parquet for large typed output). It explicitly states that agents cannot raise desktop-owned limits, choose extended deadlines, approve actions, open/run editor drafts, or turn result pages into complete exports.

- [x] **E16-T6 Add right-workspace Query Activity monitoring**
  - Depends on: E16-T1 through E16-T3
  ...
  - Notes: Added the approved header-adjacent Activity button and right-workspace Queries/Agents views while preserving the mounted editor/Profile/Quality state. Visible-only non-overlapping 500 ms polling returns bounded summary DTOs without SQL or rows; exact immutable desktop/agent SQL loads only on selection and supports Copy SQL plus a non-executing editor draft. Query rows distinguish queue/run/cancellation/terminal/result/cleanup state, bounded timings, shared resources, caps, cache bytes, truthful unavailable progress, targeted cancel, result release, and connection-scoped Release all. The Agents view shows authenticated sessions separately from paired clients, full IDs in details/title, heartbeat truth, query/result counts, and PID as unavailable unless independently observed. Desktop-owned Conservative/Balanced/Large/Custom analysis policies persist with hard combination validation and are snapshotted at admission; agents can observe but not raise them, and 300-second extended execution is desktop-only. Automated Rust/UI/Node/type/build/docs/engine/format gates pass; real-Tauri keyboard, focus, theme, minimum viewport, native Windows responsiveness, and accessibility review remain unclaimed for T5/user sign-off.

- [x] **E16-T7 Add Agent Monitor and investigate duplicate MCP processes**
  ...
  - Notes: Activity → Agents now shows paired-but-disconnected clients separately from each authenticated connection, full copyable profile/connection identities, grants, heartbeat/connectivity truth, queued/running/result/cache counts, and PID as unavailable unless independently verified. Agent access documents the expected one-host-transport/one-host-owned-stdio-child topology and surfaces bounded configuration diagnostics. The reviewed setup manager continues to own one exact `tarik` entry per host, refuse foreign same-name conflicts, verify/rollback managed changes, and never spawn an adapter from Tarik, impose a machine-wide singleton, enumerate continuously, or kill by executable name. No unintended duplicate launch was confirmed on this Linux host, so no speculative lifecycle fix was made. Native Windows Claude Desktop parentage/orphan reproduction remains explicitly unrun in `docs/review/E16-MULTI-QUERY.md` and is not claimed as accepted.
...
- [ ] **E16-T5 Benchmark, validate, and obtain cross-platform acceptance**
  ...
  - Automated evidence: `scripts/benchmark-e16-scheduler.py` and `docs/performance/E16-SCHEDULER-BENCHMARK.json` record a current-source release-sidecar 20-million-row physical-table/Parquet matrix under equal aggregate thread and shared-memory settings. Production-style queued pairs kept status p95 ≤0.304 ms and left zero cache residue. Benchmark-only true concurrency was 15.8% slower and used 125.2% more peak RSS for the physical table; it was also 18.8% slower for Parquet with 2.6% higher peak RSS and worse individual latency. Verdict: retain one heavy production execution; no graph/concurrency expansion. Full Linux automated gates are recorded in `docs/review/E16-MULTI-QUERY.md`. This task remains unchecked because real Claude Desktop, mixed real-host/import/export stress, native Windows x64 MSVC/WebView2/locking/process parentage, keyboard/focus/accessibility/DPI/mixed-monitor review, and user sign-off are unrun.
