...
- [x] **E7-T3 Build XYFlow query graph** — owner: lead-agent
  ...
  - Notes:
    ...
    - Estimate and Actual Flow provide an in-app fullscreen graph workspace for complex plans. It retains output tabs and inspector, remounts/refits XYFlow to the viewport, traps focus, exits via explicit control or `Esc`, restores focus on exit, and automatically leaves fullscreen when Results is selected. Empty/loading/error graph states can also be maximized.
