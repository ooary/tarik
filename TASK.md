...
- [x] **E7-T3 Build XYFlow query graph** — owner: lead-agent
  ...
  - Notes:
    ...
    - Estimate retains an in-app fullscreen graph workspace for complex plans. Actual Flow now opens a dedicated viewport-sized three-pane analysis workspace: immutable profiled SQL on the left, graph in the center, and operation explanation/metrics on the right. Node selection highlights only conservative SQL ranges in the profiled snapshot; current editor changes mark the profile stale until explicit rerun.
    - A single `Present` graph action deterministically selects the upper-left source node, opens its inspector, and focuses it at 1-1.35× zoom with a smooth 650ms fit transition. Multi-source joins use topmost then stable node ID as tie-breakers. Reduced-motion uses an instant transition. Present works in both normal and fullscreen Estimate/Actual Flow.
    ...
    - Final deferred-review interaction: Run query executes current SQL exactly once and stays on Results; Estimate explicitly captures current immutable SQL without execution and opens the bottom Estimate view; Actual Flow is an explicit toolbar action beside Query Library because it executes SQL. Estimate and Actual Flow retain independent immutable snapshots and show out-of-date state after editor changes.
